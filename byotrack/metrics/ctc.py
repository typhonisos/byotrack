...
class BioMetricsBis:
    ...
    def __init__(self, fiji_path: Union[str, os.PathLike]) -> None:
        ...
        """
        self.runner = FijiRunnerBis(fiji_path)
        self.last_metrics = (0.0, 0.0, 0.0, 0.0)

    def run(
        self, dataset: Union[str, os.PathLike], seq=1, n_digit=4, model=""
    ) -> float:
        """Run the CTC "Biological measures" plugin on the given dataset.
        ...
        outputs = self.runner.run(
            f"{dataset}/{seq:02}_RES{model}", f"{dataset}/{seq:02}_GT", n_digit
        )
        ...
        self.last_metrics = tuple(
            (output if output >= 0.0 else 0.0) for output in outputs
        )
